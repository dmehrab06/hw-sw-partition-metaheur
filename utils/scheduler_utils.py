...
# JK extra keywork arg to return full dict, default value (full_dict = False) gives original behavior.
def compute_dag_execution_time(graph: nx.DiGraph, partition_assignment: Dict[int, int],
                              verbose: bool = False, full_dict = False) -> Dict:
    """
    ...
    if partition_nodes != graph_nodes:
        ...
        if extra_nodes:
            logger.warning(f"Warning: Partition assignment contains nodes not in graph: {extra_nodes}")
        if missing_nodes:
    ...
    if verbose:
        ...
        logger.info(f"{'='*50}")
        logger.info(f"Total Makespan: {makespan:.2f}")
        logger.info(f"Hardware Makespan: {hw_makespan:.2f} (nodes: {hw_nodes})")
        logger.info(f"Software Makespan: {sw_makespan:.2f} (nodes: {sw_nodes})")
        logger.info(f"Communication Overhead: {total_comm_delay:.2f}")
        logger.info(f"Active Communication Edges: {len(active_comm_edges)}")
        logger.info(f"Nodes Completed: {len(completed_nodes)}/{len(graph.nodes())}")

        if active_comm_edges:
            logger.info(f"\nInter-partition communications:")
            for u, v, delay in active_comm_edges:
                logger.info(f"  ({u} -> {v}): {delay:.2f}")
