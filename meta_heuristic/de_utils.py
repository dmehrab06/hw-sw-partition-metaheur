...
def simulate_jade(dim, func_to_optimize, config):
    ...
    options = {'max_function_evaluations': config['jade']['iter'], 'seed_rng': 2022,'n_individuals':config['jade']['n_pop']}
...
def simulate_shade(dim, func_to_optimize, config):
    ...
    options = {'max_function_evaluations': config['shade']['iter'], 'seed_rng': 2022,'n_individuals':config['shade']['n_pop']}
