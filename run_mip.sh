...
for area in 0.3 0.5 0.7 0.9; do
    for hw in 0.1 0.3 0.5 0.7 0.9; do
        for seed in 0 1 2 3; do
            ...
            # Check username and set appropriate SBATCH configs
            if [ "$USERNAME" == "zm8bh" ]; then
                # Your configuration
                #echo "found myself"
                echo "#SBATCH -N 1" >> run_cvxpy.sbatch
                echo "#SBATCH -n 1" >> run_cvxpy.sbatch
                echo "#SBATCH --cpus-per-task=1" >> run_cvxpy.sbatch
                echo "#SBATCH -t 00:20:00" >> run_cvxpy.sbatch
                echo "#SBATCH -p bii" >> run_cvxpy.sbatch
                echo "#SBATCH --mem=64G" >> run_cvxpy.sbatch
                echo "#SBATCH -A nssac_covid19" >> run_cvxpy.sbatch
                echo "#SBATCH --job-name=run_cvxpy_mip" >> run_cvxpy.sbatch
                echo "module load miniforge" >> run_cvxpy.sbatch
                echo "conda activate hwswmetaheur" >> run_cvxpy.sbatch
                echo "python milp_eval.py -c ${config} -t cvxpy" >> run_cvxpy.sbatch
                
            else
                # Original configuration (for rm or others)
                ...
                echo "#SBATCH --mem=64G" >> run_cvxpy.sbatch
                echo "#SBATCH -t 03:40:00" >> run_cvxpy.sbatch
                echo "#SBATCH --job-name=run_cvxpy_mip" >> run_cvxpy.sbatch
                ...
                echo "conda run -n encode python milp_eval.py -c ${config} -t cvxpy" >> run_cvxpy.sbatch
            fi
