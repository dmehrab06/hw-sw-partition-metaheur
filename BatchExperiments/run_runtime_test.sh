...
CLASSICAL_SIZE="${1:-${CLASSICAL_SIZE:-large}}"
case "${CLASSICAL_SIZE,,}" in
  small)
  ...
  medium)
    ...
    ;;
  large)
    export HWSW_METHOD_RUNTIME_PROFILE="${HWSW_METHOD_RUNTIME_PROFILE:-makespan}"
    ;;
  *)
    echo "Unsupported runtime size '$CLASSICAL_SIZE'. Use: small | medium | large"
    exit 1
    ;;
esac
...
METHODS=(
  # "mip"  # Plot/runtime placeholder only; uncomment if you want the fixed 3600s MILP bar logic back.
  # "diff_gnn_order"
  # "gl25"
  "gcps"
  # "esa"
...
# Default runtime sweep is a single seed; override with SEEDS_OVERRIDE='42 43 ...'
SEEDS=(42)
...
RUN_REAL_MIP_RUNTIME="${RUN_REAL_MIP_RUNTIME:-0}"
MIP_RUNTIME_PLACEHOLDER_SEC="${MIP_RUNTIME_PLACEHOLDER_SEC:-3600}"
if ! [[ "$RUN_REAL_MIP_RUNTIME" =~ ^(1|true|yes|on)$ ]]; then
  FILTERED_METHODS=()
...
print_banner "Starting runtime batch run"
echo "  Classical size  : ${CLASSICAL_SIZE,,}"
echo "  Runtime profile : $HWSW_METHOD_RUNTIME_PROFILE"
...
if [[ "$RUN_REAL_MIP_RUNTIME" =~ ^(1|true|yes|on)$ ]]; then
  echo "  MIP runtime     : real run enabled"
else
  echo "  MIP runtime     : plot-only placeholder (${MIP_RUNTIME_PLACEHOLDER_SEC}s)"
fi
...
echo "  Plot step      : disabled in this script; run plot_runtime.sh separately"
...
PY
    ...
    if [[ "$PARALLEL_DATASET_METHODS" =~ ^(1|true|yes|on)$ ]]; then
      ...
      if [[ "$method" == "mip" ]]; then
        echo "  Launching MIP runtime batch with shared dataset configs from $DATASET_CFG_DIR"
        CONFIG_GLOB="$DATASET_CFG_DIR/*.yaml" \
        OUTDIR="$METHOD_DIR" \
        SOLVER_TOOL="$MIP_SOLVER_TOOL" \
        FAST_MIP="$FAST_MIP" \
        ...
        echo "  Launching runtime batch for method $method with shared dataset configs from $DATASET_CFG_DIR"
        CONFIG_GLOB="$DATASET_CFG_DIR/*.yaml" \
        OUTDIR="$METHOD_DIR" \
        HWSW_METHODS="$method" \
        HWSW_OUTPUT_DIR="$METHOD_DIR" \
...
print_banner "Finished runtime batch run (${batch_elapsed_sec}s)"
echo "Results are under $OUTDIR"
echo "Run plotting separately with:"
echo "  $ROOT/BatchExperiments/plot_runtime.sh"
